...
def build_daily_recap_embed(words):
    embed = discord.Embed(title="📋 Today's Review Words")
    
    lines = []
    for i, w in enumerate(words, 1):
        lines.append(f"**{i}. {w['vocab_name']}** — {w['reading']} — {w['meaning']}")
    
    embed.description = "\n".join(lines)
    embed.set_footer(text="Run /quiz anytime to actively test yourself on these.")
    return embed
